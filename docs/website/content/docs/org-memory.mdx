...
```text
~/.zoku/orgs/{orgId}/MEMORY.md
~/.zoku/orgs/{orgId}/memory-archive/
~/.zoku/orgs/{orgId}/memory-history/
```
...
After the profile soul stack and knowledge base catalog, Zoku appends a `## Org Memory` section for every role except **viewer**.
...
When an admin approves a proposal, Zoku merges the new fact into live memory:
...
2. **Mechanical fallback** — if the LLM call fails or no provider is available, Zoku appends the fact to pinned or the recent log and removes near-duplicate bullets using token overlap.
...
| **Path** | `~/.zoku/orgs/{orgId}/MEMORY.md` | `~/.zoku/orgs/{orgId}/profiles/{profileId}/MEMORY.md` |
