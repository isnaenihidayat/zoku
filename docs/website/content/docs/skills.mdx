---
title: "Skills"
description: "Learn how reusable skills extend Zoku profiles, including bundled memory, artifact, automation, and skill-authoring workflows."
---
...
When a profile starts a chat, Zoku appends the assigned skills catalog to the agent prompt.

On each user turn, Zoku can also attach matched skill context for the current message.
...
- **Artifact path:** save the file the agent produced with `write_file` or `write_docx` under `artifacts/{filename}`, then write `{filename}.zoku-meta.json` with MIME metadata for the dashboard. Use `write_file` for text, Markdown, HTML, JSON, and code; use `write_docx` when the user wants a real Word document.
...
Zoku discovers skills from disk and syncs them into the database.
...
If you add or change skill folders manually, run skill sync so Zoku refreshes what it knows.
