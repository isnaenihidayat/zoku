---
title: "CLI"
description: "Use Zoku from the terminal — interactive chat and slash commands."
---
...
1. Starts the Zoku server if it is not already running
...
| `--theme light\|dark` | Terminal theme (also `ZOKU_THEME`) |

Set `ZOKU_ORG_ID` to skip org selection when you belong to multiple organizations.
...
Platform workers and local bridges read the Zoku local auth token from disk. To rotate it:
