...
description: "Debug the Zoku Telegram bridge worker and fix common configuration issues."
---
...
- `Another Zoku Telegram bridge is already running` — a second bridge is running (often a leftover `bun run dev:telegram` or a stale PM2 process). Stop the other one before restarting.
