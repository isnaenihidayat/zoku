...
description: "Understand how Zoku builds the final system prompt from soul files, tools, bundled system skills, and runtime context."
---

When a user starts chatting with a profile, Zoku builds one system prompt from several sources.
...
Zoku first resolves the profile's main prompt.

If the profile has soul files, Zoku uses them:
...
If there are no soul files, Zoku uses the profile's stored `systemPrompt`.
...
After the main profile prompt, Zoku may append:
...
When soul is active, Zoku tells the agent to use tools while staying in character.

## Per-turn context

On each user message, Zoku can add fresh context for that turn:
...
Right before sending the request to the LLM, Zoku appends the current date:
...
The skills catalog is always visible. When a user message matches a skill description, Zoku can attach the full skill body for that turn (`include-body-on-match: true`).
