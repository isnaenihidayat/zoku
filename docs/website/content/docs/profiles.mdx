---
title: "Profiles"
description: "See how Zoku profiles define bot behavior, soul files, memory, tools, and model selection."
---

A **profile** is an agent definition Zoku runs for a session.
...
Zoku does **not** share one global `default` bot across every org.

When an organization is created — including the first org at install — Zoku automatically:
...
If a session or channel config omits a profile id — or names one that does not exist in the active org — Zoku falls back to that org's default profile.
...
When a chat runs with a profile, Zoku builds the agent context in this order:
...
```text
~/.zoku/orgs/{orgId}/profiles/{profileId}/
```
...
Platform-admin data exports include these org/profile workspaces as part of the full Zoku data-root ZIP.
...
- **Inherited knowledge sources** for shared product references, such as the Zoku documentation
