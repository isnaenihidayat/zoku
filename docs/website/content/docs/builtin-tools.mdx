---
title: "Builtin Tools"
description: "Review the builtin tools that Zoku profiles can use, how access is controlled, and how memory, artifact, and document workflows use file tools plus bundled skills."
---
...
When the model requests multiple tool calls in one turn, Zoku can run them concurrently — but only when **every** call in the batch is marked `parallelSafe`.
...
Custom JavaScript tools under `~/.zoku/tools/` default to sequential. Export `parallelSafe = true` from the module to opt in.
...
They are not a separate builtin. Agents use `write_file` or `write_docx` with the `save-artifact` bundled skill under `artifacts/`. The skill also documents writing a `{filename}.zoku-meta.json` sidecar so the Artifacts tab shows MIME types and timestamps.
...
**Scope:** `~/.zoku/orgs/{orgId}/profiles/{profileId}/` and `~/.zoku/tools/` (custom JS modules)
...
**Scope:** Profile workspace only. Under `artifacts/`, existing files are not silently overwritten — Zoku picks a unique filename instead.
...
Each `oldText` must be present once and edits must not overlap. Zoku applies all edits against the original file, then writes the result atomically after validation.
...
**Scope:** `~/.zoku/orgs/{orgId}/profiles/{profileId}/` only. Requires `rg` (ripgrep) on PATH.

**Availability:** When assigned to the profile.

### `knowledge_base_search`

Search uploaded knowledge base documents for relevant facts. The Knowledge tab can also show inherited URL sources, such as the Zoku documentation index at `llms.txt`; use `web_fetch` on that index and on specific `.md` pages — not `knowledge_base_search`.
...
**Scope:** Extracted text files stored under `~/.zoku/orgs/{orgId}/profiles/{profileId}/knowledge-base/`.
...
**Availability:** When assigned **and** the `[email]` section in `~/.zoku/config.ini` is complete. Omitted at runtime when incomplete (`omitUnavailableBuiltinTools`).
...
| `codingAgent` | boolean | No | When true, Zoku merges coding-agent spawn env for a command that starts with a known harness binary |

**Returns:** `{ exitCode, stdout, stderr, timedOut }`

**Coding-agent spawn env:** When the command starts with a known harness binary (`codex`, `claude`, `opencode`, `pi`, or `agent`), or when `codingAgent: true` is set with such a command, Zoku merges provider passthrough env vars on the server before spawn. `codingAgent: true` without a known binary fails closed. See [Coding agent — Provider passthrough](/coding-agent#provider-passthrough). Optional `env` keys are merged on top (credential keys cannot override passthrough).

**Scope:** Profile workspace only. Do not use `bash` to create persistent tools or `.sh` wrappers — register JavaScript tools under `~/.zoku/tools/` instead.
...
Run a focused **same-profile** sub-agent for delegated work (research, review, planning, debugging). The parent receives a structured result to summarize for the user. This is a Zoku-native in-process agent loop — **not** the external coding-agent path (`bash` + `coding-agent`).
...
The `email` tool uses a deployment-global mailbox. Required keys in `~/.zoku/config.ini` under `[email]`:
...
Upload documents via the profile dashboard or API. Search only indexes extracted text from documents with `status: "ready"`. Upload path: `~/.zoku/orgs/{orgId}/profiles/{profileId}/knowledge-base/`.

### Data portability

Platform admins can export and import the whole local Zoku data root from **Settings** in the dashboard.
...
- **Profile workspace:** `~/.zoku/orgs/{orgId}/profiles/{profileId}/` (soul files, knowledge base, `artifacts/`, etc.)
- **Custom tools directory:** `~/.zoku/tools/` (follows `ZOKU_CONFIG_DIR` if set)
