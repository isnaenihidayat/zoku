...
TEMP_CONFIG="/tmp/zoku-docs-chat-screenshots-$$"
COOKIE_JAR="/tmp/zoku-docs-chat-cookies-$$.txt"
PORT=4313
BASE_URL="http://127.0.0.1:${PORT}"
SESSION=zoku-docs-chat-screenshots
SERVER_PID=""
VIEWPORT_WIDTH=1280
VIEWPORT_HEIGHT=800
...
ZOKU_CONFIG_DIR="$TEMP_CONFIG" ZOKU_PORT="$PORT" \
  bun run "$ROOT/apps/server/src/index.ts" > /tmp/zoku-docs-chat-screenshot-server.log 2>&1 &
SERVER_PID=$!
...
agent-browser --session "$SESSION" close --all 2>/dev/null || true
agent-browser --session "$SESSION" cookies set zoku_session "$SESSION_VAL" \
  --url "${BASE_URL}/" --httpOnly --sameSite Lax
agent-browser --session "$SESSION" cookies set zoku_csrf "$CSRF_VAL" \
