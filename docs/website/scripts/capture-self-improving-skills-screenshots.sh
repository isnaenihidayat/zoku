...
# Prerequisite: bun run --filter @zoku/web build
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
SCREENSHOT_DIR="$(cd "$(dirname "$0")/.." && pwd)/public/screenshots"
TEMP_CONFIG="/tmp/zoku-docs-skills-screenshots-$$"
COOKIE_JAR="/tmp/zoku-docs-skills-cookies-$$.txt"
PORT=4315
BASE_URL="http://127.0.0.1:${PORT}"
SESSION=zoku-docs-skills-screenshots
SERVER_PID=""
VIEWPORT_WIDTH=1280
VIEWPORT_HEIGHT=720
...
start_server() {
  ...
  ZOKU_CONFIG_DIR="$TEMP_CONFIG" ZOKU_PORT="$PORT" \
    bun run "$ROOT/apps/server/src/index.ts" > /tmp/zoku-docs-skills-screenshot-server.log 2>&1 &
  SERVER_PID=$!
  ...
  tail -20 /tmp/zoku-docs-skills-screenshot-server.log || true
  exit 1
}
...
stop_server
(cd "$ROOT/apps/server" && ZOKU_CONFIG_DIR="$TEMP_CONFIG" bun run scripts/seed-skill-proposal-docs.ts)
start_server
