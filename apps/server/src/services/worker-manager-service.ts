import type { WorkerLogsResponse, WorkerProcessInfo } from "@zoku/core";
import {
...
export class WorkerManagerService {
  ...
  private workerProcessEnv(): Record<string, string> {
    ...
    const serverUrl =
      process.env.zoku_SERVER_URL?.trim() || readRuntimeServerUrl() || "";

    if (serverUrl) {
      env.zoku_SERVER_URL = serverUrl;
    }

    const configDir = process.env.ZOKU_CONFIG_DIR?.trim();
    if (configDir) {
      env.ZOKU_CONFIG_DIR = configDir;
    }
