...
2. Ensure the Namaka server is configured (`~/.zoku/config.ini` or env API keys).
3. Open **Integrations → Telegram** in the web dashboard, save your bot token and profile, and copy the **pairing code**.
4. Run `bun run dev:telegram`, message your bot, and paste the pairing code once. Settings are stored in `~/.zoku/telegram/config.ini`.
...
- `ZOKU_SERVER_URL` — server base URL (default `http://127.0.0.1:4310`)
- `ZOKU_WEB_PUBLIC_URL` — public web app URL for Composio OAuth links sent in chat (e.g. `https://zoku.example.com`)
- `ZOKU_TELEGRAM_PROFILE_ID` — bot profile (default `default`)
...
Telegram’s default **Group Privacy** limits what group messages reach the bot. Zoku still applies its own local filter, so even with privacy disabled it only responds when the message is a slash command, a reply to the bot, or a real bot mention.

Session mapping is stored in `~/.zoku/telegram/chat-sessions.json`.
