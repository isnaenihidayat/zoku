import { inferArtifactMimeType } from "@zoku/core/artifact-mime";
...
export function isArtifactMetaSidecarTool(message: ChatListItem): boolean {
  ...
  if (inputPath && (inputPath.includes(".zoku-meta") || inputPath.endsWith(ARTIFACT_META_SUFFIX))) {
    return true;
  }
  ...
  if (typeof result?.path === "string") {
    return result.path.includes(".zoku-meta") || result.path.endsWith(ARTIFACT_META_SUFFIX);
  }
 ...
 * Prefers content + `.zoku-meta.json` sidecar pairs; falls back to content-only
