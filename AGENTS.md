...
User-facing docs live in `docs/website/content/docs/` (MDX). **Audience is people who use Zoku** — org admins, operators, and chat users — not contributors implementing the product.
...
One container: API + web + platform workers. Data at `/zoku/data` (`ZOKU_CONFIG_DIR`). Dashboard: http://localhost:4310
...
# Build from source and run (uses buildx; default linux/amd64 -t zoku)
...
**Org context:** every authed call except `/v1/auth/*` and `/v1/platform/*` needs `X-Org-Id` (`@zoku/client`) or `active_org_id` cookie (`POST /v1/auth/active-org`). Middleware: `org-middleware.ts`; guards: `org-guards.ts`.
...
Path: `~/.zoku/orgs/{orgId}/profiles/{profileId}/` (`getProfileSoulDir`). Load: `loadSoulStack()`; inject: `composeSoulSystemPrompt()`.
...
Path bugs (tool resolves under repo instead of `~/.zoku`) → start here. Override root: `ZOKU_CONFIG_DIR`.

| Path | Purpose |
|---|---|
| `~/.zoku/orgs/{orgId}/profiles/{profileId}/` | Profile workspace — `getProfileSoulDir()` |
| `~/.zoku/tools/*.js` | Custom JS tools — `getCustomToolsDir()` |
...
| | Built-in | Custom JS |
|---|---|---|
| Code | `packages/core/src/tools/`, `apps/server/src/tools/` | `~/.zoku/tools/*.js` |
...
**Debug:** (1) check path resolution in `~/.zoku/tools/`, (2) confirm `buildToolExecutionContext` + real `profileId`, (3) monorepo-root paths ⇒ missing `workspaceRoot`, (4) put test files in the assigned profile workspace. Super Bot authoring rules: `SUPER_BOT_SYSTEM_PROMPT` in `packages/db/src/constants.ts`.
