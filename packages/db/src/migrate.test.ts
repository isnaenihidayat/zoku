    ...
    try {
      ...
      db.exec(`
        INSERT INTO skills (
          ...
          '/tmp/.zoku/agent/skills/coding-delegation/SKILL.md',
          0,
          0,
          1,
          '2026-06-19T00:00:00.000Z',
      ...
      expect(skill.source_path).toBe("/tmp/.zoku/agent/skills/coding-agent/SKILL.md");
    } finally {
      db.close();
    }
  });
      ...
      db.exec(`
        ...
        INSERT INTO skills (
          ...
          (
            ...
            'coding-agent',
            'Coding agent',
            '/tmp/.zoku/agent/skills/coding-agent/SKILL.md',
            0,
            0,
            1,
            '2026-06-19T00:00:00.000Z',
...
describe("migration SQL hardening", () => {
  test("rejects unexpected tenant table names before SQLite can run injected ATTACH statements", () => {
    const db = new Database(":memory:");
    const attachPath = "/tmp/zoku-migrate-attach-test.sqlite";
