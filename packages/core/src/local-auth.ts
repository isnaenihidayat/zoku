...
export class LocalAuthTokenManagedExternallyError extends Error {
  constructor() {
    super(
      "Local auth token is managed by ZOKU_LOCAL_AUTH_TOKEN and cannot be rotated on disk.",
    );
...
export async function resolveLocalAuthToken(): Promise<string> {
  const envToken = process.env.zoku_LOCAL_AUTH_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
...
export async function rotateLocalAuthToken(): Promise<string> {
  if (process.env.zoku_LOCAL_AUTH_TOKEN?.trim()) {
