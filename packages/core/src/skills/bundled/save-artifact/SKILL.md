...
After writing the artifact file, write a JSON sidecar at `artifacts/{filename}.zoku-meta.json` so the dashboard shows the correct MIME type and timestamp.
...
3. `write_file` the metadata sidecar to `artifacts/{filename}.zoku-meta.json` using the same base filename from step 2.
